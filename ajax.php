...
class AuthorizationAjaxRequest extends AjaxRequest
{
    ...
    public function getcomment()
    {
        ...
        $homepage = $this->getRequestParam( "homepage" );
        ...
        if( !filter_var( $email, FILTER_VALIDATE_EMAIL ) ) {
            $this->setFieldError( "email", "Email is wrong" );
            return;
        }
        
        if ( !empty( $homepage)) {
            if( ! filter_var( $homepage, FILTER_VALIDATE_URL ) ) {
                $this->setFieldError( "homepage", "URL is wrong" );
                return;
            }
        }

        if( empty( $text ) ) {
            $this->setFieldError( "text", "Confirm the text" );
            return;
        }

        if( $text != strip_tags( $text ) ) {
            $this->setFieldError( "text", "Text contains tags" );
            return;
        }
        ...
        try {
            $new_comment = $user->createComment( $user_id, $email, $homepage, $text, $ip, $browser, $date);
        }
        catch( \Exception $e ) {
        ...
        $this->message = sprintf( "Hello, %s! Thank you for comment." );
    ...
    public function register()
    {
        ...
        if( ! preg_match( "/[A-Za-z0-9]/", $username ) ) {
            $this->setFieldError( "username", "Only latinskie i chisla" );
            return;
        }
