...
class User
{
    ...
    public function getInfoFromDB($offset = 0, $order = 'username'){
        $query = "select * from comments order by {$order} limit {$offset}, 2";
        $sth = $this->db->prepare( $query );
        $sth->execute();
        $row = $sth->fetchAll();
        if( ! $row ) {
    ...
    public function createComment($user_id, $email, $homepage, $text, $ip, $browser, $date) {
        $username = $this->getUsername($user_id);
        
        $query = "insert into comments (username, email, homepage, text, ip, browser, datetime)
            values (:username, :email, :homepage, :text, :ip, :browser, :datetime)";
